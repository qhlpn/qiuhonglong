...
+ **相关算法**
  ...
  + **标记阶段**
    ...
    + **引用计数法**
      ...
      **A：**计数器是强引用计数器，使用 **弱引用** 区分可解决循环引用。若 A 强引用 B，则 B 引用 A 就降为弱引用，当判断是否为无用对象时，仅考虑强引用计数是否为 0，不关心弱引用计数。当强引用计数减至 0 时销毁引用的对象。
  ...
  + **清除阶段**
    ...
    + **标记-清除**
      ...
      + **标记**：Collector从引用根节点遍历，标记所有被引用对象（**可达对象！不是垃圾对象**）
    ...
    + **复制**
      ...
      > 将内存空间逻辑上划分为两块，垃圾回收时将 **正在使用的内存** 的存活对象复制到 **未被使用的内存** 中，之后清除正在使用的内存的所有对象，最后交换两个内存的角色。（解决 标记-清除 算法碎片缺点）
      >
      > 缺点：需要两倍的内存空间
